...
if (typeof window.onload != 'function')
  ...
  def bounding_points
    ...
    @markers.each do |marker| 
  		if maxlat.nil? || marker.lat > maxlat then maxlat = marker.lat end
  		if minlat.nil? || marker.lat < minlat then minlat = marker.lat end
  		if maxlon.nil? || marker.lon > maxlon then maxlon = marker.lon end 
  		if minlon.nil? || marker.lon < minlon then minlon = marker.lon end
  	end
